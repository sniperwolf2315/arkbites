...
function addEcheance( $date )
{
	...
	$y2  = $y1+10;
	$mid = $y1 + (($y2-$y1) / 2);
	$this->RoundedRect($r1, $y1, ($r2 - $r1) + 10, ($y2-$y1), 2.5, 'D');
	$this->Line( $r1, $mid, $r2 + 10, $mid);
	$this->SetXY( $r1 + ($r2 - $r1)/2 - 5 , $y1+1 );
	$this->SetFont( "Arial", "B", 10);
	$this->Cell(10 + 10,4, "FECHA DE VENCIMIENTO", 0, 0, "C");
	$this->SetXY( $r1 + ($r2-$r1)/2 - 5 , $y1 + 5 );
	$this->SetFont( "Arial", "", 10);
	$this->Cell(10,5,$date, 0,0, "C");
}
...
{
	$this->SetFont( "Arial", "", 10);
	$length = $this->GetStringWidth( "Observaciones : " . $remarque );
	$r1  = 10;
	$r2  = $r1 + $length;
	$y1  = $this->h - 45.5;
	$y2  = $y1+5;
	$this->SetXY( $r1 , $y1 );
	$this->Cell($length,4, "Observaciones : " . $remarque);
}
...
function addCadreEurosFrancs($subtotal, $iva, $total)
{
	$r1  = $this->w - 70;
	$r2  = $r1 + 60;
	$y1  = $this->h - 40;
	$y2  = $y1+20;
	$this->RoundedRect($r1, $y1, 40, ($y2-$y1), 2.5, 'D');
	$this->Line( $r1+20,  $y1, $r1+20, $y2); // avant EUROS
	$this->Line( $r1+20, $y1+4, $r2 - 20, $y1+4); // Sous Euros & Francs
	...
	$this->SetFont( "Arial", "", 8);
	$this->Cell(17,4, $subtotal . " " . EURO, 0, 0, "C");
	$this->SetXY( $r1, $y1+10 );
	$this->Cell(20,4, "IVA", 0, 0, "C");
	$this->SetFont( "Arial", "", 8);
	$this->Cell(17,4, $iva . " " . EURO, 0, 0, "C");
	$this->SetXY( $r1, $y1+15 );
	$this->Cell(20,4, "TOTAL", 0, 0, "C");
	$this->SetFont( "Arial", "", 8);
	$this->Cell(17,4, $total . " " . EURO, 0, 0, "C");
}
...
function addTVAs( $params, $tab_tva, $invoice )
{
	...
	if ( $params["AccompteExige"] == 1 )
	{
		...
		else if ( $params["accompte_percent"] > 0 )
		{
			...
			/*if ( strlen ($params["Observaciones"]) == 0 )
				$this->addRemarque( "" );
			else
				$this->addRemarque( $params["Observaciones"] );*/
		}
		else
			$this->addRemarque( "Dr�le d'acompte !!! " . $params["Observaciones"]);
	}
	else
	{
		if ( strlen ($params["Observaciones"]) > 0 )
